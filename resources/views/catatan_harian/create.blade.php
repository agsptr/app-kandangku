...
@section('content')
    <div class="container">
        <h1 class="mb-4">Tambah Catatan Harian</h1>

        <form action="{{ route('catatan_harian.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="form-control @error('kegiatan') is-invalid @enderror" id="kegiatan"
                    name="kegiatan" value="{{ old('kegiatan') }}" required>
                @error('kegiatan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="form-control @error('kondisi_bebek') is-invalid @enderror" id="kondisi_bebek"
                    name="kondisi_bebek" value="{{ old('kondisi_bebek') }}" required>
                @error('kondisi_bebek')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
